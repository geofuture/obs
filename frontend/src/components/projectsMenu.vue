      ...
      <v-list-group sub-group value="true" dense>
        ...
        <v-list-item v-for="(p, index) in projects" :key="index" link dense>
          ...
          <v-list-item-action>
            <v-tooltip right>
              <template v-slot:activator="{ on }">
                <v-icon
                  @click="switchDeleteDialogOn({ name: p.name, id: p.id })"
                  v-on="on"
                  >mdi-delete</v-icon
                >
              </template>
          ...
          <v-list-item-action>
            <v-tooltip right>
              <template v-slot:activator="{ on }">
                <v-icon v-on:click="() => loadProject(p.tableName)" v-on="on"
                  >mdi-layers</v-icon
                >
              </template>
    ...
    <v-list-group prepend-icon="mdi-map" value="true" dense>
      ...
      <v-list-group no-action sub-group value="true" dense>
        ...
        >
          ...
          <v-list-item-action>
            <v-tooltip right>
              <template v-slot:activator="{ on }">
                <v-icon @click="removeProject(layer.tableName)" v-on="on"
        ...
        <v-list-item link dense>
          ...
          <v-list-item-action>
            <v-tooltip right>
              <template v-slot:activator="{ on }">
                <v-icon
                  v-if="wmsVisible == false"
                  v-on="on"
                  @click.prevent="wmsSwhitchOn"
                  >mdi-eye-off</v-icon
                >
                <v-icon v-else v-on="on" @click.prevent="wmsSwhitchOff"
                  >mdi-eye</v-icon
                >
              </template>
      ...
      <template>
        ...
        <v-list-item id="v-step-5" link>
          ...
          <v-list-item-action @click="switchWmsDialogOn">
            <v-tooltip right>
              <template v-slot:activator="{ on }">
...
export default {
  ...
  methods: {
    ...mapMutations([
      "switchWmsDialogOn",
      "switchDownloadDialogOn",
      "switchDeleteDialogOn",
      "wmsSwhitchOn",
      "wmsSwhitchOff"
    ]),
